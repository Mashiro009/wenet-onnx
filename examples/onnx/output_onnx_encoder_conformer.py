...
def check_encoder_onnx_and_pytorch(encoder_model,encoder_model_path):
    ...
    offset = torch.tensor(1,dtype=torch.int64)
    required_cache_size = torch.tensor(-1,dtype=torch.int64)
    subsampling_cache=torch.zeros(1,1,256)
    elayers_output_cache=torch.zeros(12,1,1,256)
    conformer_cnn_cache=torch.zeros(12,1,256,15)
    chunk_onnx_outputs = []
    for i in range(5):
    ...
    subsampling_cache=torch.zeros(1,1,256)
    elayers_output_cache=torch.zeros(12,1,1,256)
    conformer_cnn_cache=torch.zeros(12,1,256,15) # 在卷积cache这里使用zeros是必要的
...
def main():
    ...
    encoder_model_path=os.path.join(output_dir,'encoder_chunk_conformer.onnx')
